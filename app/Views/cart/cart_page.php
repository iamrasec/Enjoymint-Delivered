                              ...
                              <?php if (in_array($product['pid'], $discount_id)){ ?>
                                <?php for($x = 0; $x<count($discount_data); $x++){
                                  if($discount_data[$x]['id'] === $product['pid']){
                                    ?>
                                  <?php if($discount_data[$x]['promo_type'] == "bxgx"){ ?>
                                         <strong class="total-price-display" style="color:red;">Free</strong><br>
                                  ...
                                  <?php } else {?>                                 
                                   ...
                                   <strong class="total-price-display" style="color:red;">$<?= ($product['product_data']->price * $product['qty']) - $pricesubtotal; ?></strong><br>
                                  <input type="hidden" class="product-total-price product-<?= $product['pid']; ?>-total-price" value="<?= number_format($product['product_data']->price * $product['qty'], 2, '.', ''); ?>" data-pid="<?= $product['pid']; ?>">
                                  <strong class="total-price-display" style="color: #3E413C; text-decoration: line-through;">$<?= number_format($product['product_data']->price * $product['qty'], 2, '.', ','); ?></strong>
                                <?php
                                  }  
                                  ...
                                  <?php 
                                 foreach($sale_discount as $discount){
                                  if($discount['saleTotal'] == $product['product_data']->price && $discount['id'] === $product['pid'] ){ 
                                    ?>
                                  <strong class="total-price-display" style="color:#42413d;background-color: #3BAD07;padding-left:5px; padding-right:5px; border-radius: 4px;">-$<?= $discount['dis_sale'] * $product['qty']; ?></strong><br>
                                  <strong class="total-price-display" style="color:red;">$<?= $discount['saleTotal'] * $product['qty']; ?></strong><br>
                                  <input type="hidden" class="product-total-price product-<?= $product['pid']; ?>-total-price" value="<?= number_format($product['product_data']->price * $product['qty'], 2, '.', ''); ?>" data-pid="<?= $product['pid']; ?>">
                                  <strong class="total-price-display" style="color: #3E413C; text-decoration: line-through;">$<?= number_format($discount['origPrice'] * $product['qty'], 2, '.', ','); ?></strong>
...
update_cart_count();
  ...
  $(document).ready(function() {
    ...
    var subtotalprice = 0;
    <?php if(!empty($pricesubtotal)) : ?>
    var promo_discount = <?= $pricesubtotal;
     ?>;
    <?php else: ?>
    ...
    $(".product-total-price").each(function() {
      ...
      if(!priceSub && !promo_discount){ 
        subtotal += parseFloat($(this).val());
      }else{
        subtotalprice += parseFloat($(this).val());
        subtotal = subtotalprice - promo_discount;
